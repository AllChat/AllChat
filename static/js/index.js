...
var Account = {
    createNew: function() {
        ...
        account.setColor = function() {
            $("#color-select-icon").on("click", function(event){
                $("#color").click();
            });
            $("#color").change(function(event){
                font_color = $(this).val();
                $(".chatBox-msg").css("color", font_color);
                $("#chat-input textarea").css("color", font_color);
            });
        };
        ...
        account.textareaSubmit = function() {
            ...
            $("#chat-input").children("button").eq(0).on("click",function(event){
                $(this).siblings("textarea").val('');
            });
        ...
        account.addContent = function(id, name, content, timestr) {
            ...
            var $dd = $("<dd></dd>").addClass("chatBox-msg").css("font-size",font_size+"px").css("font-family",font_family).css("color",font_color).html(content);
        ...
        account.constructInfoDialog = function(){
            ...
            $head.append($('<p></p>').text('头像')).append('<img/>');
            ...
            $submit = $("<div class='btn-submit btn-primary'>确认</div>").attr('id','personal-info-submit');
            $table = $("<div class='dialog-table'></div>");
            $table.append($head).append($nickname).append($email).append($submit);
            $('#personal-info-list').append($table);
            ...
            $head.on("click", "img" , function(event){
                event.stopPropagation();
                $displayIcons = $('<div></div>').attr('id','head-options');
                for (var i=0;i<6;i++){
                ...
                $('#personal-info-list div.dialog-table').css("display","none");
                ...
                $('#head-options img').on('click',function(event){
                    ...
                    $('#personal-info-list div.dialog-table').css("display","block");
                });
            });
        }
        ...
        account.constructPasswordDialog = function(){
            $oldpassword = $('<div></div>').addClass('password-input');
            ...
            $submit = $("<div class='btn-submit btn-primary'>确认</div>").attr('id','change-password-submit');
            $table = $("<div class='dialog-table'></div>");
            $table.append($oldpassword).append($newpassword).append($newconfirm).append($submit);
            $('#change-password-list').append($table);
            $submit.on('click',function(event){
                event.stopPropagation();
        ...
        account.refresh_chat_history_list = function($chat_selector_li){
            if($chat_selector_li.attr("class")!="selected_date"){
                ...
                $.each(chat_history, function(index, val) {
                    ...
                    $dd = $("<dd></dd>").addClass("chatBox-msg").css("font-size",font_size+"px").css("font-family",font_family).css("color",font_color).html(chat_content);
        ...
        account.getMsg = function() {
            if(typeof(Worker)!=="undefined") {
                var worker =new Worker("/static/js/getmsg.js");
